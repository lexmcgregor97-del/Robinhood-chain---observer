...
export function normalizeExecutionIntent(input) {
  ...
  const spendAsset = String(input.spendAsset || "").toLowerCase();
  if (spendAsset !== NATIVE && !ADDRESS.test(spendAsset)) throw new Error("invalid-spend-asset");
  const spendAmount = String(input.spendAmount ?? "");
  if (!UINT.test(spendAmount) || spendAmount === "0") throw new Error("invalid-spend-amount");
  ...
  return Object.freeze({
    ...
    from,
    to,
    valueWei, spendAsset, spendAmount,
    data,
