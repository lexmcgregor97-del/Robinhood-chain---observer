# Atlas Micro-Mainnet Receipt Recovery — Review Packet

## Scope

Review branch `fix/micro-mainnet-recovery`, based on accepted durable execution
state revision 2 (`459e3b7da574290d8ebde130650ffd14b09553d7`). Production remains V6b
`PAPER_ONLY`. The change adds `execution-recovery.js`, an offline
`recover-executions.js` command, tests, and documentation. The dormant
lifecycle delegates its recovery method to the same engine.
...
3. It verifies `eth_chainId == 4663` and every pending record's chain ID before
   requesting receipts.
4. A confirmed or reverted receipt must have a recognized status and, when the
   provider returns it, the exact requested transaction hash. The journal final
   transition is appended/fsynced and the complete state is atomically
   checkpointed before nonce finalization is attempted. Nonce finalization is a
   second evidence-first checkpoint.
...
7. Immediately before every durable mutation, the command re-reads the state
   checkpoint and requires its `savedAt`, evidence sequence, and evidence hash
   to match its in-memory view. A concurrent writer blocks before append.
...
check, checkpoint revalidation, and explicit operator assertion prevent
concurrent operation with the web runtime. The runbook forbids configuring the
confirmation string on the web service.

## Tests

Tests cover confirmed and reverted receipts, recent missing receipts, stale
manual-review escalation, reservations without signed bytes, provider errors,
invalid receipt status, receipt-hash mismatch, final-record nonce residue, and
a full real evidence-journal plus atomic-state round trip through the isolated
command. Provider error text is not returned.
...
2. Implement journaled exact-approval orchestration, including zero-first and
   residue cleanup.
3. Implement the concrete per-intent preflight reads for policy state,
   WETH/native balances, daily spend, and post-buy sell simulation.
4. Implement the reviewed epoch-migration tool.
...
2. Can a receipt from the wrong chain, hash, or with an unknown status become a
...
5. Can the one-shot process race the web runtime or continue mutating after a
   persistence failure?
