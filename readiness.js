export function assessReadiness({ latestBlock, cursor, backfillActive, lastError }) {
  ...
  const cursorAhead = latestBlock > 0 && cursor > latestBlock;
  const synchronized = latestBlock > 0 && !backfillActive && cursor === latestBlock;
  ...
  if (cursorAhead) reasons.push("scanner-updating");
